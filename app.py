...
# --- Page Config ---
st.set_page_config(page_title="TrustLens | Secure Notary", page_icon="ðŸ›¡ï¸", layout="wide")
...
def login_page():
    st.markdown("<h1 style='text-align: center; color: #0056b3;'>ðŸ›¡ï¸ TrustLens</h1>", unsafe_allow_html=True)
...
def home_dashboard():
    st.markdown(f"<h2 style='color: #0056b3;'>Welcome to the Trust Dashboard</h2>", unsafe_allow_html=True)
    st.markdown("Your digital notary and immutable provenance engine.")
    
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("#### ðŸ”’ Secure Upload")
        ...
        if st.button("ðŸš€ Upload Document Now", use_container_width=True, type="primary"):
            st.switch_page("views/1_Upload_Document.py")
            
    with col2:
        st.markdown("#### âœ… Instant Verification")
        ...
        if st.button("ðŸ” Verify a Document", use_container_width=True):
...
# --- Navigation Setup ---
login_p = st.Page(login_page, title="Log In", icon="ðŸ”")
verify_p = st.Page("views/2_Verify_Document.py", title="Public Verification", icon="âœ…")
home_p = st.Page(home_dashboard, title="Home Dashboard", icon="ðŸ ")
upload_p = st.Page("views/1_Upload_Document.py", title="Secure Upload", icon="ðŸ“¤")
analytics_p = st.Page("views/3_Trust_Analytics.py", title="Analytics Dashboard", icon="ðŸ“Š")
...
st.markdown("""
    <div style='text-align: center; color: #6c757d; font-size: 12px; margin-top: 50px; padding-top: 20px; border-top: 1px solid #e9ecef;'>
        Â© 2026 TrustLens | <a href='https://github.com/GokavalasaHemanthNaidu/TrustLens-Visual-Document-Trust-Chain' target='_blank'>GitHub Repo</a> | v1.4.0
    </div>
